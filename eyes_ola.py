...
thread.start_new_thread(olaThread, (adc, adcValue))
...
irisMap   = pi3d.Texture(IRIS  , mipmap=False,
              filter=pi3d.GL_LINEAR)
scleraMap = pi3d.Texture(SCLERA, mipmap=False,
              filter=pi3d.GL_LINEAR, blend=True)
lidMap    = pi3d.Texture(LID   , mipmap=False,
              filter=pi3d.GL_LINEAR, blend=True)
...
#		print(frames/(now-beginningTime))
	...
	if JOYSTICK_X_IN >= 0 and JOYSTICK_Y_IN >= 0 and adcValue[JOYSTICK_X_IN]>0.0:
...
while True:
	...
	if PUPIL_IN >= 0 and adcValue[PUPIL_IN]>0.0: # Pupil scale from sensor
		v = adcValue[PUPIL_IN]
